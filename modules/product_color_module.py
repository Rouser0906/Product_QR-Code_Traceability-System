...
class ProductColorModule(QWidget):
    def __init__(self):
        super().__init__()
    ...
    def initUI(self):
        ...
        self.table.setHorizontalHeaderLabels(["产品颜色", "自定义", "操作"])
    ...
    def setup_permissions(self, user):
        ...
        if not can_create:
            self.addBtn.setToolTip("您没有新增产品颜色的权限")
            
        # 检查删除权限
        can_delete = has_permission(user, "product.delete")
        self.delBtn.setEnabled(can_delete)
        if not can_delete:
            self.delBtn.setToolTip("您没有删除产品颜色的权限")
            
        # 检查下载权限
        can_download = has_permission(user, "product.download")
        self.downloadBtn.setEnabled(can_download)
        if not can_download:
            self.downloadBtn.setToolTip("您没有下载产品颜色的权限")
            
        # 检查打印权限
        can_print = has_permission(user, "product.print")
        self.printBtn.setEnabled(can_print)
        if not can_print:
            self.printBtn.setToolTip("您没有打印产品颜色的权限")
            
        print(f"产品颜色模块权限设置完成 - 用户: {user.get('username', 'unknown')}, 查看: {can_view}, 新增: {can_create}, 删除: {can_delete}, 下载: {can_download}, 打印: {can_print}")
    ...
    def refresh_table(self):
        try:
            conn = self.get_conn()
            cursor = conn.cursor()
            cursor.execute("SELECT name, IFNULL(remark, '') FROM product_colors ORDER BY id")
            ...
            import traceback
            print(f"[ERROR] 刷新产品颜色表格出错: {e}")
            print(traceback.format_exc())
            QMessageBox.critical(self, "错误", f"刷新产品颜色表格出错: {e}")

    def add_color(self):
        dialog = EditProductColorDialog(parent=self)
        if dialog.exec_() == QDialog.Accepted:
            data = dialog.get_data()
            try:
                conn = self.get_conn()
                cursor = conn.cursor()
                cursor.execute("INSERT INTO product_colors (name, remark) VALUES (?, ?)", (data['name'], data['remark']))
                conn.commit()
                conn.close()
                QMessageBox.information(self, "成功", "产品颜色添加成功！")
            except Exception as e:
                QMessageBox.warning(self, "失败", f"添加产品颜色失败: {e}")
            self.refresh_table()

    def delete_color(self):
        row = self.table.currentRow()
        if row < 0:
            QMessageBox.warning(self, "提示", "请先选中要删除的产品颜色！")
            return
        name = self.table.item(row, 0).text()
        reply = QMessageBox.question(self, "确认删除", f"确定要删除产品颜色：{name} 吗？", QMessageBox.Yes | QMessageBox.No)
        if reply == QMessageBox.Yes:
            try:
                conn = self.get_conn()
                cursor = conn.cursor()
                cursor.execute("DELETE FROM product_colors WHERE name=?", (name,))
                conn.commit()
                conn.close()
                QMessageBox.information(self, "成功", "产品颜色删除成功！")
            except Exception as e:
                QMessageBox.warning(self, "失败", f"删除产品颜色失败: {e}")
            self.refresh_table()

    def edit_color(self, row_data):
        dialog = EditProductColorDialog(parent=self)
        dialog.setWindowTitle("编辑产品颜色")
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            try:
                conn = self.get_conn()
                cursor = conn.cursor()
                cursor.execute("UPDATE product_colors SET name=?, remark=? WHERE name=?", (data['name'], data['remark'], row_data[0]))
                conn.commit()
                conn.close()
                QMessageBox.information(self, "成功", "产品颜色修改成功！")
            except Exception as e:
                QMessageBox.warning(self, "失败", f"修改产品颜色失败: {e}")
            self.refresh_table()
    ...
    def on_table_cell_clicked(self, row, column):
        ...
        if column == 2:  # 操作列
            ...
            if item and item.text() == "编辑":
                ...
                self.edit_color(row_data)

class EditProductColorDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("新增产品颜色")
        self.resize(300, 120)
        ...
        layout.addRow("产品颜色", self.name_edit)
