...
function Start-FileWatcher {
    ...
    $hsWatcher.Path = Join-Path $LocalBasePath "demo_json_a"
    $hsWatcher.Filter = "*.json"
    $hsWatcher.NotifyFilter = [System.IO.NotifyFilters]::CreationTime -bor [System.IO.NotifyFilters]::LastWrite
...
function Sync-AllFiles {
    ...
    # 同步demo_json_a文件
    $hsDir = Join-Path $LocalBasePath "demo_json_a"
    if (Test-Path $hsDir) {
        $hsFiles = Get-ChildItem -Path $hsDir -Filter "*.json" -File
        Write-SyncLog "找到 $($hsFiles.Count) 个demo_json_a文件"
