...
try {
  ...
  # Show monitor paths
  $aPath = Join-Path $projectRoot "cloud\demo_json_a"
  $zyPath = Join-Path $projectRoot "cloud\demo_json_b"
  Write-Host "Monitor Paths:" -ForegroundColor Cyan
  Write-Host "  A JSON: $aPath -> C:\inetpub\qr-system\companies\demo_json_a" -ForegroundColor White
  Write-Host "  B JSON: $zyPath -> C:\inetpub\qr-system\companies\demo_json_b" -ForegroundColor White
  
  # Create monitor directories
  New-Item -ItemType Directory -Force -Path $aPath | Out-Null
