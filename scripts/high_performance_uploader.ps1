...
foreach ($candidate in @('cloud/demo_json_a','companies/demo_json_a')) {
    ...
    if (Test-Path $fullPath) { $LocalHS = $fullPath; break }
}
foreach ($candidate in @('cloud/demo_json_b','companies/demo_json_b')) {
    $fullPath = Join-Path $ProjectRoot $candidate
    if (Test-Path $fullPath) { $LocalZY = $fullPath; break }
}

$RemoteHS = '/companies/demo_json_a'
$RemoteZY = '/companies/demo_json_b'

Write-Log "目录映射: LocalHS=$LocalHS, LocalZY=$LocalZY" "INFO"
...
while ($true) {
    ...
    try {
        ...
        # 并行同步两个目录
        if ($LocalHS) {
            Sync-DirectoryParallel -LocalDir $LocalHS -RemoteDir $RemoteHS -Filter 'HS-Q*.json'
        }
        if ($LocalZY) {
            Sync-DirectoryParallel -LocalDir $LocalZY -RemoteDir $RemoteZY -Filter 'ZY-Q*.json'
        }
