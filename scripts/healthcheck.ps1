...
try {
  $resp = Invoke-WebRequest -Method GET -Uri ("{0}/index.html?id={1}" -f $BaseUrl, ($Code?$Code:'A-DEMO-000000001')) -MaximumRedirection 0 -UseBasicParsing -TimeoutSec 15
  $redirOk = $false
} catch {
...
$apiUrl = ("{0}/api/get_product_data.php?code={1}" -f $BaseUrl, ($Code?$Code:'A-DEMO-000000001'))
try {
...
# 4) JSON 访问 + CORS/MIME
$companyDir = if ($Company -eq 'a') { 'demo_json_a' } else { 'demo_json_b' }
$jsonUrl = ("{0}/companies/{1}/{2}.json" -f $BaseUrl, $companyDir, ($Code?$Code:'A-DEMO-000000001'))
