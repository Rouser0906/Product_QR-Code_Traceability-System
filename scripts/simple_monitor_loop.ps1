...
Write-Host "[$(Get-Date -Format 'HH:mm:ss')] 监控目录: $LocalA" -ForegroundColor Cyan
Write-Host "[$(Get-Date -Format 'HH:mm:ss')] 监控目录: $LocalB" -ForegroundColor Cyan
    ...
    try {
        ...
        if (Test-Path $LocalA) {
            ...
            foreach ($file in $aFiles) {
                if (-not $processedFiles.ContainsKey($file.FullName) -or $processedFiles[$file.FullName] -lt $file.LastWriteTime) {
                    Write-Host "[$(Get-Date -Format 'HH:mm:ss')] 发现新的A文件: $($file.Name)" -ForegroundColor Yellow
        ...
        if (Test-Path $LocalB) {
            ...
            foreach ($file in $zyFiles) {
                if (-not $processedFiles.ContainsKey($file.FullName) -or $processedFiles[$file.FullName] -lt $file.LastWriteTime) {
                    Write-Host "[$(Get-Date -Format 'HH:mm:ss')] 发现新的B文件: $($file.Name)" -ForegroundColor Yellow
