# 稳定的FTP上传守护进程 - 专门解决A-DEMO-000010232.json上传问题
param(
...
Write-Log "Local A Dir: $aDir"
Write-Log "Local B Dir: $zyDir"

# 立即上传A-DEMO-000010232.json（如果存在）
$targetFile = Join-Path $aDir "A-DEMO-000010232.json"
if (Test-Path $targetFile) {
  Write-Log "Found target file: A-DEMO-000010232.json"
  $result = Upload-ToFTP -localFile $targetFile -remoteDir "/companies/demo_json_a"
  if ($result) {
    Write-Log "✓ A-DEMO-000010232.json uploaded successfully to cloud server!"
  } else {
    Write-Log "✗ Failed to upload A-DEMO-000010232.json"
  }
}
...
Get-ChildItem -Path $aDir -Filter "*.json" | ForEach-Object {
