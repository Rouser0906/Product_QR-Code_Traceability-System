...
New-Item -ItemType Directory -Force -Path $aDir, $zyDir, $logDir | Out-Null
...
Write-SmartLog "Local A Dir: $aDir"
Write-SmartLog "Local B Dir: $zyDir"
...
$aResult = Scan-ForMissingFiles -localDir $aDir -remoteDir "/companies/demo_json_a" -prefix "A"
$zyResult = Scan-ForMissingFiles -localDir $zyDir -remoteDir "/companies/demo_json_b" -prefix "B"

Write-SmartLog "A Results: Total=$($aResult.Total), Missing=$($aResult.Missing), Uploaded=$($aResult.Uploaded), Skipped=$($aResult.Skipped)" "SUCCESS"
Write-SmartLog "B Results: Total=$($zyResult.Total), Missing=$($zyResult.Missing), Uploaded=$($zyResult.Uploaded), Skipped=$($zyResult.Skipped)" "SUCCESS"
...
while ($true) {
    ...
    if (Test-Path $aDir) {
        $aNewFiles = Get-ChildItem -Path $aDir -Filter "*.json" | Where-Object { $_.LastWriteTime -gt $lastScanTime }
        foreach ($file in $aNewFiles) {
            $newFiles += @{ Path = $file.FullName; RemoteDir = "/companies/demo_json_a"; Name = $file.Name }
        }
    }
    
    # Check B directory for new files
    if (Test-Path $zyDir) {
