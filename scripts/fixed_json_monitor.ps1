...
Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Monitoring A: $LocalA" -ForegroundColor Cyan
Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Monitoring B: $LocalB" -ForegroundColor Cyan
Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Press Ctrl+C to stop" -ForegroundColor Yellow
Write-Host ""

# Get existing files to avoid re-uploading
if (Test-Path $LocalA) {
    Get-ChildItem -Path $LocalA -Filter "*.json" -File | ForEach-Object {
        $processedFiles[$_.FullName] = $_.LastWriteTime
    }
}
if (Test-Path $LocalB) {
    Get-ChildItem -Path $LocalB -Filter "*.json" -File | ForEach-Object {
        $processedFiles[$_.FullName] = $_.LastWriteTime
    }
}
    ...
    try {
        ...
        if (Test-Path $LocalA) {
            ...
            foreach ($file in $aFiles) {
                if (-not $processedFiles.ContainsKey($file.FullName) -or $processedFiles[$file.FullName] -lt $file.LastWriteTime) {
                    Write-Host "[$(Get-Date -Format 'HH:mm:ss')] New A file: $($file.Name)" -ForegroundColor Yellow
        ...
        if (Test-Path $LocalB) {
            ...
            foreach ($file in $zyFiles) {
                if (-not $processedFiles.ContainsKey($file.FullName) -or $processedFiles[$file.FullName] -lt $file.LastWriteTime) {
                    Write-Host "[$(Get-Date -Format 'HH:mm:ss')] New B file: $($file.Name)" -ForegroundColor Yellow
