...
function Test-QueueAlert {
    $hsCount = if (Test-Path "cloud\demo_json_a") { 
    ...
    $totalQueue = $hsCount + $zyCount
    
    if ($totalQueue -gt 150) {  # 超过150个文件告警
        $message = "文件队列过长: $totalQueue 个文件 (HS:$hsCount, ZY:$zyCount)"
        Write-AlertLog $message "ALERT"
...
function Show-AlertStatus {
    ...
    # 文件队列
    $hsCount = if (Test-Path "cloud\demo_json_a") { (Get-ChildItem "cloud\demo_json_a" -Filter "*.json").Count } else { 0 }
