param(
  ...
  [string]$LocalHS = '',  # 自动检测项目根\cloud\demo_json_a
  [string]$LocalZY = '',  # 自动检测项目根\cloud\demo_json_b
...
# 自动检测本地监控目录
if (-not $LocalHS) { $LocalHS = Join-Path $ProjectRoot 'cloud\demo_json_a' }
if (-not $LocalZY) { $LocalZY = Join-Path $ProjectRoot 'cloud\demo_json_b' }
...
# 创建本地监控目录
New-Item -ItemType Directory -Force -Path $LocalHS | Out-Null
New-Item -ItemType Directory -Force -Path $LocalZY | Out-Null
...
Write-Log "File watcher starting. HS=$LocalHS ZY=$LocalZY RemoteHS=$RemoteHS RemoteZY=$RemoteZY FTP=$FtpHost`:$FtpPort"
$w1 = New-Watcher -path $LocalHS
$w2 = New-Watcher -path $LocalZY
