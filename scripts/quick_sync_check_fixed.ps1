...
function Show-SyncStatus {
    ...
    $aInfo = Get-DirectoryInfo $LocalA "A"
    $zyInfo = Get-DirectoryInfo $LocalB "B"
    
    Write-Host "Local Directory Status:" -ForegroundColor White
    
    foreach ($info in @($aInfo, $zyInfo)) {
...
function Test-UploadFunction {
    ...
    $testFile = Join-Path $LocalA "TEST-$(Get-Date -Format 'yyyyMMdd-HHmmss').json"
    
    try {
        # Ensure directory exists
        if (-not (Test-Path $LocalA)) {
            New-Item -ItemType Directory -Force -Path $LocalA
        }
