param(
  ...
  [string[]]$LocalHSCandidates = @('cloud/demo_json_a','companies/demo_json_a'),
  [string[]]$LocalZYCandidates = @('cloud/demo_json_b','companies/demo_json_b'),
  [string[]]$RemoteHSCandidates = @('/companies/demo_json_a'),
  [string[]]$RemoteZYCandidates = @('/companies/demo_json_b'),
...
$LocalHS = Resolve-LocalDir -candidates $LocalHSCandidates -root $root
$LocalZY = Resolve-LocalDir -candidates $LocalZYCandidates -root $root
$RemoteHS = Select-RemoteDir -candidates $RemoteHSCandidates
$RemoteZY = Select-RemoteDir -candidates $RemoteZYCandidates

Write-Log "FTP Uploader starting. Host=$FtpHost Port=$FtpPort"
Write-Log "LocalHS=$LocalHS LocalZY=$LocalZY RemoteHS=$RemoteHS RemoteZY=$RemoteZY"
$ftpOk = Test-FtpConnectivity
if (-not $ftpOk) { Write-Log "WARN: FTP connectivity test failed at startup. Will retry in loop." }

while ($true) {
  try {
    Sync-Dir -localDir $LocalHS -remoteDir $RemoteHS -filter 'HS-Q*.json'
    Sync-Dir -localDir $LocalZY -remoteDir $RemoteZY -filter 'ZY-Q*.json'
  } catch {
