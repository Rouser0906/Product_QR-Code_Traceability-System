...
## 本地电脑端路径要求（必须严格遵守）
- **HS系列JSON文件必须存放在**: `C:\Projects\Demo\cloud\demo_json_a\`
- **ZY系列JSON文件必须存放在**: `C:\Projects\Demo\cloud\demo_json_b\`
- **生成规则**: 本地电脑通过系统打印二维码标签后分别生成的HS-Q*.JSON和ZY-Q*.JSON文件只能且必须分别存放在上述指定目录下

## 云服务器端路径要求（必须严格遵守）
- **服务器物理路径**:
  - `C:\inetpub\qr-system\companies\demo_json_a\` (HS系列JSON文件)
  - `C:\inetpub\qr-system\companies\demo_json_b\` (ZY系列JSON文件)
- **对应 FTP 虚拟路径**:
  - `/companies/demo_json_a/` (HS系列)
  - `/companies/demo_json_b/` (ZY系列)

## 自动同步要求（必须几乎同时完成）
- 本地JSON文件生成后，必须几乎在同时被自动同步上传到云服务器对应目录
- **HS-Q*.json**: `C:\Projects\Demo\cloud\demo_json_a\` → `C:\inetpub\qr-system\companies\demo_json_a\`
- **ZY-Q*.json**: `C:\Projects\Demo\cloud\demo_json_b\` → `C:\inetpub\qr-system\companies\demo_json_b\`
...
目标：
- 历史与未来所有二维码 JSON（HS-Q*.json、ZY-Q*.json）中的 "verification_url" 统一为：
